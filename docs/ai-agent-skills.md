...
- 长期记忆：`src/services/ai/agentMemory.ts`
- 用户自定义 Skill：`src/services/ai/agentCustomSkills.ts`
...
- 平台事实问题：涉及资产、机柜、IP、告警、审计、虚拟服务器等内容时，先调用平台工具/API，再让模型总结。
- 通用问题：不需要查询平台事实时，直接调用当前启用模型回答，例如 qwen3.6-35b、DeepSeek、Gemini 等。
...
/memory                  查看长期记忆
/clear-memory            清空长期记忆
/skill add 名称:说明      新增用户自定义 Skill
/custom-skills           查看用户自定义 Skill
/tools                   查看只读 Agent API 工具
/skills                  查看内置 Skill
```

第一版记忆和自定义 Skill 存在浏览器本地存储，适合快速验证交互。后续桌面版和多人版建议迁移到 SQLite：

```text
agent_memories(id, content, tags, created_at, updated_at)
agent_skills(id, name, description, rules, enabled, created_at, updated_at)
knowledge_documents(id, title, source_type, content, metadata, created_at)
knowledge_chunks(id, document_id, content, embedding_ref, metadata)
agent_runs(id, question, mode, model, status, created_at)
agent_steps(id, run_id, type, tool_name, input_summary, output_summary, status, duration_ms)
```
...
- 数据中心日常运维规范。
- 服务器、存储、交换机、安全设备维修经验。
- 卓豪、Prometheus、CMDB、ZStack MCP 的 API 调用说明。
- 告警处理 SOP、巡检记录、复盘报告。
- AI 助手从历史处理过程中沉淀出的可复用经验。
