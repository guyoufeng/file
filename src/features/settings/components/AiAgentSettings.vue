...
const apiKeys = ref<AgentApiKeyView[]>([]);
const apiKeyName = ref("外部 Agent 集成");
const apiKeyScope = ref<"read" | "write">("read");
...
const writeCurlExample = computed(() => {
  const token = createdApiToken.value || "<write-api-key>";
  return `curl -X POST -H "Authorization: Bearer ${token}" -H "Content-Type: application/json" -d "{\\"computerName\\":\\"QF-SRV-NEW\\",\\"businessIp\\":\\"10.10.9.9\\"}" "${apiBaseUrl.value}/devices"`;
});
...
async function refreshApiKeys() {
  try {
    apiKeys.value = await loadAgentApiKeys();
  } catch {
    apiKeys.value = [];
  }
}
...
async function createApiKey() {
  const scopes = apiKeyScope.value === "write" ? ["read", "write"] : ["read"];
  try {
    const created = await createServerAgentApiKey({
      name: apiKeyName.value.trim() || "外部 Agent 集成",
      scopes: scopes as Array<"read" | "write">,
    });
    createdApiToken.value = created.token;
    await refreshApiKeys();
    message.value = "Agent API Key 已创建。请立即保存完整 Token，页面刷新后只显示脱敏预览。";
  } catch (error) {
    message.value = error instanceof Error ? error.message : "创建 Agent API Key 失败";
  }
}
</script>
...
<template>
  <section class="agent-panel">
        ...
        <div class="gateway-layout">
          ...
          <div class="gateway-qr">
            <div
              v-if="gatewayPairing"
              class="qr-grid"
              aria-label="消息网关配对二维码"
              :style="{ gridTemplateColumns: `repeat(${gatewayPairing.qrSize}, 1fr)`, gridTemplateRows: `repeat(${gatewayPairing.qrSize}, 1fr)` }"
            >
            ...
            <code v-if="gatewayPairing">{{ gatewayPairing.pairingUrl }}</code>
          </div>
        </div>
    ...
    <section class="agent-api-panel">
      ...
      <div class="token-box api-key-box" aria-label="Agent API Key 管理">
        <div>
          <strong>Agent API Key</strong>
          <span>支持 read 只读 Key 和 read/write 写入 Key。写入 Key 可调用设备、进出记录和变更记录写入接口。</span>
        </div>
        <div class="token-controls api-key-controls">
          <input
            v-model="apiKeyName"
            type="text"
            placeholder="Key 名称"
            aria-label="Agent API Key 名称"
          />
          <select v-model="apiKeyScope" aria-label="Agent API Key 权限">
            <option value="read">只读</option>
            <option value="write">读写</option>
          </select>
          <button type="button" @click="createApiKey">创建 Key</button>
          <button type="button" @click="refreshApiKeys">刷新</button>
        </div>
        <code v-if="createdApiToken" class="created-token">新 Token：{{ createdApiToken }}</code>
        <ul class="compact-list api-key-list">
          <li v-for="key in apiKeys" :key="key.id">
            <strong>{{ key.name }}</strong>
            <span>{{ key.scopes.join(" / ") }} · {{ key.tokenPreview }} · {{ key.enabled ? "启用" : "停用" }}</span>
          </li>
          <li v-if="apiKeys.length === 0">暂无 API Key。</li>
        </ul>
      </div>
...
.api-key-controls {
  grid-template-columns: minmax(180px, 1fr) 110px repeat(2, auto);
}
