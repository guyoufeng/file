...
import { getAccessRecords, type AccessRecord } from "../../features/access-management/accessRecords";
import { getChangeEvents, type ChangeEvent } from "../../features/change-management/changeEvents";
import { getConnectionRecords, type ManagedConnection } from "../../features/connection-manager/connections";
...
function isDcimQuestion(question: string, rooms: Room[], racks: Rack[]) {
  ...
  if (/机房|机柜|服务器|设备|资产|告警|报警|异常|故障|责任人|用途|业务ip|带外ip|u位|位置|在哪|计算机名|固定资产|编号|sn|SN|序列号|维保|硬件配置|操作系统|负责人|负责|查询|查下|查看|看下|搜索|虚拟机|虚拟服务器|云主机|宿主|审计|操作记录|历史记录|导入记录|查询记录|接线|连线|端口|交换机|线缆|变更|上架|下架/.test(question)) {
    return true;
  }
...
export async function answerWithAiAssistant(
  ...
  const toolResult = runDeterministicAiQuery(
    ...
    request.accessRecords ?? getAccessRecords(),
    request.changeEvents ?? getChangeEvents(),
    request.connectionRecords ?? getConnectionRecords(),
  );
