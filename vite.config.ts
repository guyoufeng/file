      ...
      server.middlewares.use('/__ai_proxy/models', async (req, res) => {
        ...
        try {
          ...
          if (body.provider === 'gemini') {
            res.setHeader('Content-Type', 'application/json')
            res.end(JSON.stringify({ data: body.model ? [{ id: body.model }] : [] }))
            return
          }
          ...
          if (body.provider === 'ollama') {
            ...
            const models = Array.isArray(data.models) ? data.models.map((item: { name: string }) => item.name) : []
            res.setHeader('Content-Type', 'application/json')
          ...
          res.end(
            JSON.stringify({
              message: error instanceof Error ? error.message : '模型代理请求失败',
            }),
          )
        }
      })
